...
class GalleryWidgetkitHelper extends WidgetkitHelper {
	...
	*/
	...
	/*
		Function: upload_files_gallery
			Upload files to gallery

		Returns:
			Void
	*/
	public function upload_files_gallery() {

		$files = isset($_FILES["files"]) ? $_FILES["files"]:array();
		$path  = $this['request']->get('path', 'string');
		$uploaded = array();
		
		if (isset($files["name"]) && strlen($path)) {

			$targetpath = $this['path']->path('media:'.$path);

			for ($i=0;$i<count($files["name"]);$i++) {
				
				if (!$files["error"][$i] &&preg_match("/image/i", $files["type"][$i])) {
					$uploaded[] = $files["name"][$i];

					copy($files["tmp_name"][$i], $targetpath."/".$files["name"][$i]);
				}
			}
		}
...
$widgetkit['event']->bind('task:upload_files_gallery', array($widgetkit['gallery'], 'upload_files_gallery'));
